# Checklist de publicação — Jornada360

## Antes da publicação web
- [x] Dados fictícios
- [x] Avisos de independência institucional
- [x] Health Score separado de risco de crédito
- [x] Prioridades explicáveis
- [x] CSAT/NPS demonstrativos
- [x] Conclusão/interrupção demonstrativas
- [x] Testes automatizados
- [ ] Teste de usabilidade humano
- [ ] Capturas finais das telas
- [ ] Acabamento visual final
- [ ] Revisão final do README
- [ ] Definição da URL pública

## Não publicar
- dados reais de clientes;
- CPF, RG, renda, endereço ou documentos pessoais;
- credenciais;
- critérios proprietários de instituição financeira;
- promessas de aprovação;
- informações internas de antigos empregadores.
